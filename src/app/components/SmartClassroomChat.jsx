...
const ChatPage = () => {
  ...
  const fetchParticipants = async (subjectId) => {
    ...
    try {
      setLoading(true);
      const res = await axiosInstance.get(`${config.API_BASE}subjects/${subjectId}/details/`);
      setParticipants(isStudent ? res.data.teachers : dummyStudent);
    } catch (err) {
  ...
  useEffect(() => {
    if (!isStudent) setParticipants(dummyStudent);
    fetchSubjects();
  }, [fetchSubjects]);
          ...
          <Card className="flex-grow-1 d-flex flex-column h-100">
            <Card.Header>
              {selectedUser ? (
                <div>
                  Chat with{" "}
                  <strong>{selectedUser.name || `${selectedUser.first_name} ${selectedUser.last_name}`}</strong>
                </div>
              ) : (
