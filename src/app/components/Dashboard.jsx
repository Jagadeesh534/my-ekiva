  ...
  useEffect(() => {
    const fetchSchoolStats = async () => {
      try {
        const response = await axiosInstance.get(
          config.API_BASE + "school_stats?school_id=" + school.id
        );
    ...
    const fetchStudentSubjects = async () => {
      try {
        const response = await axiosInstance.get(
          config.API_BASE +
            "subjects/subject_by_student/?student_id=" +
            userInfo.id
        );
        console.log("Fetched student subjects data:", response);
        const subjects = response.data.map((subject) => ({
          id: subject.id,
          name: subject.name,
          image: `https://ui-avatars.com/api/?name=` + subject.name,
        }));
  ...
  return (
    <div className="dashboard-container">
      ...
      <h2 className="dashboard-title">
        Hello {userInfo?.first_name}! Welcome to My Ekiva
      </h2>
      {loginType === "student" && (
      ...
      <div
        ...
        style={loginType === "student" ? { marginTop: "4rem" } : {}}
      >
